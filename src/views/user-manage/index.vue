...
// 处理导入用户后数据不重新加载的问题
onActivated(getListData) // 当keep-alive缓存的组件被激活时会调用该函数，否则缓存的组件内容不变
...
const onImportExcelClick = () => {
  router.push('/user/import') // 跳转到导入页面
}
...
watch(roleDialogVisible, val => {
  ...
  // 每次打开角色对话框后，通过请求获得了selectUserId。后续再打开同一个对话框时selectUserId不为空，不会再发送请求。
  // 置空可以是得每次都发送请求
})
